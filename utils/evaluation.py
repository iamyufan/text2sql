...
def compute_metrics(
    ...
    Inputs:
        * gt_query (str): The path to the ground-truth SQL queries corresponding to the text prompts
        * model_query (str): The path to SQL queries generated by the model, conditioned on the same text prompts
        * gt_records (str): If provided, it should be a path to a pickle file containing a list of records
                                  returned by the ground-truth SQL queries.
        * model_records (str): If provided, it should be a path to a pickle file containing a list of records
                                     returned by the model-generated SQL queries.
    """
    gt_qs, gt_records, _ = _load_queries_and_records(gt_query, gt_records)
    model_qs, model_records, model_error_msgs = _load_queries_and_records(
        model_query, model_records
    )

    sql_em = _compute_sql_exact_match(gt_qs, model_qs)
    record_em = _compute_record_exact_match(gt_records, model_records)
    record_f1 = _compute_record_F1(gt_records, model_records)
...
def save_queries_and_records(sql_queries: List[str], sql_path: str, record_path: str):
    ...
    records, error_msgs = __compute_records(sql_queries)
    with open(record_path, "wb") as f:
        pickle.dump((records, error_msgs), f)


def _load_queries_and_records(sql_path: str, record_path: str):
    """
    ...
    """
    processed_qs = _read_queries(sql_path)

    if record_path is not None:
        with open(record_path, "rb") as f:
            records, error_msgs = pickle.load(f)
    else:
        records, error_msgs = __compute_records(processed_qs)

    return processed_qs, records, error_msgs


def _read_queries(sql_path: str):
    with open(sql_path, "r") as f:
        qs = [q.strip() for q in f.readlines()]
    return qs


def __compute_records(processed_qs: List[str]):
    """
    ...
    for i, query in enumerate(processed_qs):
        futures.append(pool.submit(_compute_record, i, query))
...
def _compute_record(query_id, query):
...
def _compute_sql_exact_match(gt_qs: List[str], model_qs: List[str]):
    """
...
def _compute_record_exact_match(gt_records: List[Any], model_records: List[Any]):
    """
...
def _compute_record_F1(gt_records: List[Any], model_records: List[Any]):
    """
